...
python tools/inventory_target.py /path/to/local/target \
  --target-id waves-base-001 \
  --product "Pocket Monsters Waves" \
  --region TBD \
  --language TBD \
  --version TBD \
  --update base \
  --output manifests/local/waves-base-001.json
```
...
python tools/map_extracted_target.py /path/to/extracted/target \
  --target-id waves-base-001 \
  --output manifests/local/waves-base-001.executable-map.json
```
